...
describe("TreeCards Integration", () => {
  ...
  it("should relocate triggered cards with injection markers to target sections", () => {
    const contextWithInjection = `### Preamble

World Lore:
### Character Info

Some character info here.

<inject section="preamble" />
## Trap Instructions

Traps need to be resolved by a rogue.
...
[Author's note: Be creative.]`;
    ...
    const cardsWithInjection: StoryCard[] = [
      {
        id: "1",
        keys: ["Character"],
        entry: "### Character Info\n\nSome character info here.",
        type: "lore",
      ...
      },
      {
        ...
        entry:
          '<inject section="preamble" />\n## Trap Instructions\n\nTraps need to be resolved by a rogue.',
        type: "lore",
        title: "Trap Instructions",
        description: "",
      },
    ];

    const ctx = parseContext(contextWithInjection, cardsWithInjection, 10000);
    const config = TreeCards.validateConfig({ enable: "true" });
    const hookContext = createHookContext(cardsWithInjection);

    const result = onContext(ctx, config, hookContext);

    expect(result.preamble).toContain("## Trap Instructions");
    expect(result.preamble).toContain("Traps need to be resolved by a rogue");

    const worldLore = getSection(result, "World Lore")?.body ?? "";
    expect(worldLore).not.toContain("Trap Instructions");
    expect(worldLore).not.toContain("<inject");
    expect(worldLore).toContain("Character Info");
  });

  it("should handle injection markers in triggered cards to different sections", () => {
    const contextWithMultipleInjections = `### Preamble
...
[Author's note: Note.]`;
    ...
    const cardsWithInjections: StoryCard[] = [
      {
        id: "1",
        keys: ["memory"],
    ...
    const ctx = parseContext(
      contextWithMultipleInjections,
      cardsWithInjections,
      10000
    );
    const config = TreeCards.validateConfig({ enable: "true" });
    const hookContext = createHookContext(cardsWithInjections);
    ...
    expect(result.postamble).toContain("Add this to the end");

    const worldLore = getSection(result, "World Lore")?.body ?? "";
    expect(worldLore).not.toContain("Remember this");
    expect(worldLore).not.toContain("Add this to the end");
  });
});
